...
export function navigateReducer(state, action) {
    ...
    if (forceOptimisticNavigation && (prefetchValues == null ? void 0 : prefetchValues.kind) !== PrefetchKind.TEMPORARY) {
        ...
        const data = createRecordFromThenable(fetchServerResponse(url, optimisticTree, state.nextUrl));
        ...
        const res = fillCacheWithDataProperty(temporaryCacheNode, state.cache, optimisticFlightSegmentPath, ()=>data, true);
    ...
    if (!prefetchValues) {
        const data = createRecordFromThenable(fetchServerResponse(url, state.tree, state.nextUrl));
